...
- **Statute Currency Checker:** `python -m app.check_statute_currency` is an offline, LLM-free static linter over `backend/app/templates/*.json`, run manually (not yet wired into CI). `statute_registry.json` is the maintained list of known law-recodification events (currently: the BNS/BNSS/BSA criminal-law cutover, 1 July 2024; the four Labour Codes' nationwide notification, 21 Nov 2025) — add a new entry there whenever India recodifies a law a template might cite, rather than hardcoding it in the script. It flags `OUTDATED` (a superseded statute cited with no mention of its replacement anywhere in the template — exits non-zero) and `HEDGED` (the replacement *is* mentioned, but only next to hedge language like "where notified"/"still transitioning" that treats its adoption as uncertain rather than settled — advisory, this is the exact shape labour_commissioner.json's original bug took). Each template also carries an optional `last_reviewed` date; missing or >180-day-old dates get an advisory `NEVER REVIEWED`/`REVIEW DUE` warning — the original 6 templates (lokayuktha, nhrc, womens_commission, rti, consumer_forum, district_court) currently have none and are due for a legal-accuracy pass.
...
- Statute currency check: `cd backend && python -m app.check_statute_currency` (no server needed)
